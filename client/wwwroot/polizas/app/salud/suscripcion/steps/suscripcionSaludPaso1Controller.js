...
define(['angular', 'constants', 'lodash', 'mpfPersonConstants', 'saludFactory', '/scripts/mpf-main-controls/components/ubigeo/component/ubigeo.js', 'saludSuscripcionFactory', 'constantsSalud', 'mpfPersonComponent'], function(ng, constants, _, personConstants) {
  ...
  suscripcionSaludPaso1Controller.$inject = [
    '$scope'
    , '$stateParams'
    , 'saludLists'
    , 'saludFactory'
  ...
  function suscripcionSaludPaso1Controller(
    $scope
    , $stateParams
    , saludLists
    , saludFactory
    ...
    , mModalConfirm) {
     ...
     $scope.$on('$locationChangeSuccess',function(evt, newUrl, oldUrl) {
      ...
      function _validarDatosListaNegra() {
        ...
        if($scope.firstStep.Contratante.Correo) reqLN.push({ "tipo": "CORREO", "valor": $scope.firstStep.Contratante.Correo });
        if($scope.firstStep.Contratante.TelefonoMovil) reqLN.push({ "tipo": "TLF_MOVIL", "valor": $scope.firstStep.Contratante.TelefonoMovil });
        if($scope.firstStep.Contratante.TelefonoCasa) reqLN.push({ "tipo": "TLF_FIJO", "valor": $scope.firstStep.Contratante.TelefonoCasa });
        if($scope.firstStep.Titular.Correo && ($scope.firstStep.Titular.Correo !== $scope.firstStep.Contratante.Correo)) reqLN.push({ "tipo": "CORREO", "valor": $scope.firstStep.Titular.Correo });
        if($scope.firstStep.Titular.TelefonoMovil && ($scope.firstStep.Titular.TelefonoMovil !== $scope.firstStep.Contratante.TelefonoMovil)) reqLN.push({ "tipo": "TLF_MOVIL", "valor": $scope.firstStep.Titular.TelefonoMovil });
        if($scope.firstStep.Titular.TelefonoCasa && ($scope.firstStep.Titular.TelefonoCasa !== $scope.firstStep.Contratante.TelefonoCasa)) reqLN.push({ "tipo": "TLF_FIJO", "valor": $scope.firstStep.Titular.TelefonoCasa });
          ...
          if(response.OperationCode === constants.operationCode.success) {
            ...
            response.Data.forEach(function(element) {
              if(element.Resultado) {
                var elemetLN = {
                  codAplicacion: personConstants.aplications.SALUD,
                  numCotizacion: $stateParams.quotationNumber,
                ...
                switch(element.Tipo) {
                  case "CORREO": 
                    var fuente = element.Valor === $scope.firstStep.Contratante.Correo ? 'contratante' : 'asegurado';
                    msg += "El correo del " + fuente + " est&aacute; en la tabla de Cliente/ Unidad inelegible por estudios t&eacute;cnicos.<br/>"; 
                    break;
                  case "TLF_MOVIL": 
                    var fuente = element.Valor === $scope.firstStep.Contratante.TelefonoMovil ? 'contratante' : 'asegurado';
                    ...
                    break;
                  case "TLF_FIJO": 
                    var fuente = element.Valor === $scope.firstStep.Contratante.TelefonoCasa ? 'contratante' : 'asegurado';
                    msg += "El tel&eacute;fono fijo del " + fuente + " est&aacute; en la tabla de Cliente/ Unidad inelegible por estudios t&eacute;cnicos.<br/>"; 
                    break;
                  default: "";
                }
              }
            ...
            if(_validarFormulario() && msg !== "") {
              _bloquearFraudulento(msg, null, datosLN);
            } else if(!_validarFormulario()) {
              var formMsg = 'Tiene errores en el rellenado del formulario';
              $scope.status.open1 = true;
              $scope.status.open2 = true;
              if($scope.firstStep.Asegurados.length > 0) $scope.status.open3 = true;
              
              if(msg !== "") {
                _bloquearFraudulento(msg, formMsg, datosLN);
              } else {
                mModalAlert.showError(formMsg, 'Error');
              }
            } else {
              _guardarData();
            }
      ...
      function _bloquearFraudulento(message, extra, datosLN) {
        var fullMsg = '';
        if(extra) fullMsg += extra + '<br/>';
        fullMsg += message;
  ...
  return ng.module('appSalud')
    .controller('suscripcionSaludPaso1Controller', suscripcionSaludPaso1Controller)
    .factory('loaderSuscripcionP1Controller', ['saludFactory', '$q', 'mainServices',
