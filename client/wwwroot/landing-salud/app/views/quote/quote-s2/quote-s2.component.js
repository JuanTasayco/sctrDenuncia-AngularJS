...
define(['angular', 'lodash', 'constantsSalud', 'saludFactory', 'question'], function(
  ...
  QuoteS2Controller.$inject = [
    '$scope', '$uibModal', 'mModalAlert', '$stateParams', 'saludSuscripcionFactory', 'saludFactory', 'oimPrincipal', '$state', '$filter'
  ];

  function QuoteS2Controller($scope, $uibModal, mModalAlert, $stateParams, saludSuscripcionFactory, saludFactory, oimPrincipal, $state, $filter) {
    var vm = this;
    ...
    vm.$onInit = function() {
      ...
      console.log($stateParams.quotationNumber)
      _getUserProfile();
    ...
    $scope.paso3 = function () {
      $state.go('quote.steps', {quotationNumber: $scope.firstStep.NumeroDocumento ? $scope.firstStep.NumeroDocumento : $scope.firstStep.NumeroDocumentoEncript, step: 3});
    }
    ...
    var _validarPermiso = function (firstStep) {
      ...
      if (firstStep.EstadoSolicitud != ''  && $scope.motivoSolicitud.indexOf("DPS") < 0) {
        if ($scope.motivoSolicitud.length > 0){
          $state.go('quote.steps', {quotationNumber: firstStep.NumeroDocumentoEncript, step: 3});
        }else{
    ...
    function _validarPaso(data) {
      if ($scope.motivoSolicitud.indexOf("DPS") < 0) {
        if ($scope.motivoSolicitud.length > 0){
          $state.go('quote.steps', {quotationNumber: data.NumeroDocumentoEncript, step: 3});
        }else if (data.Paso > 2 ) {
    ...
    $scope.$on('$locationChangeSuccess',function(evt, newUrl, oldUrl) {
      if (oldUrl.split('/').pop() !== 2) {
        $state.go('quote.steps', {quotationNumber: $stateParams.quotationNumber, step: 2});
      }
     });

    var _getUserProfile = function () {
      $scope.userName = 'usersalud';
    }
    ...
    var _formatPreguntas = function (cbGuardarPaso2) {
      var data = {
        ...
        NumeroDocumentoEncript: $scope.firstStep.NumeroDocumentoEncript,
        Paso: "2",
        Cuestionario: {
  ...
  return angular.module('appLanding')
    .controller('QuoteS2Controller', QuoteS2Controller);
});
