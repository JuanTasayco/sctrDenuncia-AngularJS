define(["angular", "/grqc/app/factory/grqcFactory.js"], function (ng) {
  ...
  grqcBandejaRequerimientosController.$inject = ["$scope", "$rootScope", "grqcFactory", "$q", "$state",
    "mModalAlert", "mModalConfirm", "oimClaims", "$http", '$filter', 'mpSpin'];

  function grqcBandejaRequerimientosController($scope, $rootScope, grqcFactory, $q, $state,
    mModalAlert, mModalConfirm, oimClaims, $http, $filter, mpSpin) {
      ...
      $http.post(        
        $scope.API_POLIZAS + 'api/cobranza/decesos/requerimientos/ejecutivoCobranza/0', 
        {
          ...
          /*if($scope.REQUERIMIENTOS.length > 0){
            if($scope.REQUERIMIENTOS[0].cod_usua != $scope.USUARIO){
              $scope.REQUERIMIENTOS = [];
    ...
    $scope.FN_GRABAR = function(){
      $scope.REQ.fec_opcional_ini = formatearFecha($scope.REQ.fec_opci_ini);
      $scope.REQ.fec_opcional_fin = formatearFecha($scope.REQ.fec_opci_fin);
      ...
      if($scope.REQ_TIPO == 'REQ AUTOSERVICIO'){
        $scope.REQ.num_poliza = '0';
      }else{
        if(!$scope.REQ.poliza.validacion){        
          mModalAlert.showWarning('Completar campo: Número de Póliza', 'Formulario');
          return false;
        }
      }
      if($scope.REQ.cod_cia == null || $scope.REQ.cod_cia == undefined){        
        mModalAlert.showWarning('Completar campo: Compañía', 'Formulario');
        return false;
      }
      if($scope.REQ.cod_sector == null || $scope.REQ.cod_sector == undefined){        
        mModalAlert.showWarning('Completar campo: Sector', 'Formulario');
        return false;
      }
      if($scope.REQ.id_motivo == null || $scope.REQ.id_motivo == undefined){        
        mModalAlert.showWarning('Completar campo: Motivo', 'Formulario');
        return false;
      }  

      //var submotivos = $filter('filter')($scope.LS_REQ_SUBMOTIVOS, { ID_MOTIVO: $scope.REQ.id_motivo }, true);
      if($scope.REQ.tiene_submotivos){
        if($scope.REQ.id_submotivo == null || $scope.REQ.id_submotivo == undefined || $scope.REQ.id_submotivo == 0){     
          mModalAlert.showWarning('Completar campo: Submotivo', 'Formulario');
          return false;
        }  
      }
      else{
        $scope.REQ.id_submotivo = 0;
      }

      if($scope.REQ.requiere_placa){
        if($scope.REQ.nom_placa == null || $scope.REQ.nom_placa == undefined || $scope.REQ.nom_placa == ""){        
          mModalAlert.showWarning('Completar campo: Placa', 'Formulario');
          return false;
        }  
      }
      else{
      ...
      if($scope.REQ.requiere_fechas){
        if($scope.REQ.fec_opcional_ini == null || $scope.REQ.fec_opcional_ini == undefined || $scope.REQ.fec_opcional_ini == ""){        
          mModalAlert.showWarning('Completar campo: Fecha Inicio', 'Formulario');
          return false;
        }  
        if($scope.REQ.fec_opcional_fin == null || $scope.REQ.fec_opcional_fin == undefined || $scope.REQ.fec_opcional_fin == ''){        
          mModalAlert.showWarning('Completar campo: Fecha Fin', 'Formulario');
          return false;
        }  
      }
      else{
        $scope.REQ.fec_opcional_ini = "";
        $scope.REQ.fec_opcional_fin = "";
      }

      if($scope.REQ.info_adicional == null || $scope.REQ.info_adicional == undefined){        
        $scope.REQ.info_adicional = '';
      } 
      
      if($scope.REQ.info_adicional.trim() == ''){        
        mModalAlert.showWarning('Completar campo: Sustento', 'Formulario');
        return false;
      }       

      /*CONFIRMACIÓN*/
      mModalConfirm.confirmInfo('¿Estás seguro(a) que desea registrar el Requerimiento?','REQUERIMIENTO','').then(function(result) {
        console.log('result');
        console.log(result);
        if (result) {
          var fd = new FormData();
          /*REQUEST*/
          var request = {
            cod_cia: $scope.REQ.cod_cia,
            cod_sector: $scope.REQ.cod_sector,
            tip_gestor: '0',
            cod_gestor: '0',
            cod_usua: $scope.USUARIO.toUpperCase(),
            tipo_flujo: $scope.REQ_TIPO,
            id_motivo: $scope.REQ.id_motivo,
            id_submotivo: $scope.REQ.id_submotivo,
            nom_placa: $scope.REQ.nom_placa.toUpperCase(),
            fec_opcional_ini: $scope.REQ.fec_opcional_ini,
            fec_opcional_fin: $scope.REQ.fec_opcional_fin,
            info_adicional: $scope.REQ.info_adicional.toUpperCase()
          }
          ...
          mpSpin.start();
          $http.post(        
            $scope.API_POLIZAS + 'api/cobranza/deceso/' + $scope.REQ.num_poliza + '/ejecutivoCobranza/0/requerimiento', 
            fd,
            {
              transformRequest: [],
            ...
            if(data.cod == '200'){
              var id_req = data.msj || 'NN'; 
              mModalAlert.showSuccess('Registro Correcto: Requerimiento #' + id_req, 'Registro').then(function(result) {
                $scope.FN_VISTA_LISTAR(true);
              })['catch'](function (e) {
                $scope.FN_VISTA_LISTAR(true);
              });              
            }else{
              mModalAlert.showError('No se pudo registrar el requerimiento', 'Error');
            }      
          })['catch'](function (e) {
            mpSpin.end();
            mModalAlert.showError('Ocurrió un problema', 'Error');
    ...
    $scope.openCalendarIni = function () {
      $scope.popup_fec_ini.opened = true;
    };
    $scope.openCalendarFin = function () {
      $scope.popup_fec_fin.opened = true;
    };

    $scope.FN_MOTIVO_CHANGE = function(){
      if($scope.REQ_TIPO == 'REQ AUTOSERVICIO'){  
    ...
    $scope.FN_VISTA_NUEVO = function(tipo_flujo){
      $scope.VISTA = 'NUEVO';
      $scope.REQ_TIPO = tipo_flujo;
      $scope.REQ = {
        num_poliza: null,
        cod_cia: null,
        cod_sector: null,
      ...
      if($scope.REQ_TIPO == 'REQ OBSERVACION'){
        $scope.REQ.dias_permitidos = $scope.REQ_OBSE_CONFIG.ca_dias_ejecut_emis || -1;
      } else if($scope.REQ_TIPO == 'REQ GESTION'){
        $scope.REQ.dias_permitidos = $scope.REQ_GEST_CONFIG.ca_dias_ejecut_emis || -1;
      } else if($scope.REQ_TIPO == 'REQ AUTOSERVICIO'){
        $scope.REQ.dias_permitidos = $scope.REQ_AUTO_CONFIG.ca_dias_ejecut_emis || -1;
      }
    ...
    $scope.FN_LISTAS = function(){
      ...
      /*$scope.LS_REQ_SUBMOTIVOS = [
        { "id_motivo": 71212, "id_submotivo": 1, "nom_submotivo": "CONECTIVIDAD" },
        { "id_motivo": 71212, "id_submotivo": 2, "nom_submotivo": "EQUIPO" },
        { "id_motivo": 71212, "id_submotivo": 3, "nom_submotivo": "LIQUIDACION" },
        { "id_motivo": 71212, "id_submotivo": 4, "nom_submotivo": "OTROS" },
        { "id_motivo": 71213, "id_submotivo": 5, "nom_submotivo": "CONEXION" },
        { "id_motivo": 71213, "id_submotivo": 6, "nom_submotivo": "RENOVACION" },
        { "id_motivo": 71213, "id_submotivo": 7, "nom_submotivo": "OTROS" },
        { "id_motivo": 71215, "id_submotivo": 8, "nom_submotivo": "REGISTRO" },
        { "id_motivo": 71215, "id_submotivo": 9, "nom_submotivo": "MODIFICACION" }
      ];*/
    ...
    $scope.FN_POLIZA_VALIDAR = function(){
      if($scope.REQ_TIPO == 'REQ AUTOSERVICIO' || $scope.VISTA == 'LISTAR'){
        return false;
      }

      if($scope.REQ.num_poliza == null || $scope.REQ.num_poliza == ''){
        return false;
      }

      $scope.REQ.poliza = {};
      $scope.REQ.poliza.validacion = false;

      console.log('Validando Póliza...');
      console.log($scope.REQ.num_poliza);     
      ...
      .then(function (response) {
        ...
        if(polizas){
          if(polizas.length > 0){
            ...
            // FECHA EMISION REQUERIMIENTO
            var fecha_emision = $scope.REQ.poliza.fec_actu.substring(0,10) || '';
            var fecha_parts = fecha_emision.split('/');

            if(fecha_parts.length == 3){
              fecha_emision = new Date(fecha_parts[2], fecha_parts[1] - 1, fecha_parts[0]); 
            }
            ...
            var diffDays = (getBusinessDateCount(fecha_emision, $scope.FECHA) -1);
            ...
            mModalAlert.showWarning('Póliza No Encontrada', 'Validación');
          }
        }else{
          mModalAlert.showError('Póliza no se pudo validar', 'Validación');
        }      
      })['catch'](function (e) {
        mpSpin.end();
        mModalAlert.showError('Ocurrió un problema', 'Error');
    ...
    $scope.FN_INICIO = function () {
      ...
      $scope.popup_fec_ini = { opened: false };  
      $scope.popup_fec_fin = { opened: false };
  ...
  //$scope.MimeType2.set(".msg", "application/vnd.msoutlook");
  //$scope.MimeType2.set(".msg", "application/octet-stream");
  ...
  return ng.module("appGrqc")
    .controller("GrqcBandejaRequerimientosController",grqcBandejaRequerimientosController)
    ;
});
