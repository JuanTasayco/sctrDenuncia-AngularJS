define([
  ...
  var appGrqc = angular.module('appGrqc');

  appGrqc.factory('grqcFactory', ['proxyGeneral', 'proxyLookup', 'proxyClinic', 'proxyAffiliate', 'proxyDiagnostic', 'proxyDoctor', 
    ...
    'proxyPowerEpsProduct', 'proxyPowerEpsAffiliate', 'proxyProduct', 'proxyCoverageMinsa', 'proxySinisterExistence', 'proxyHighCosts', 'proxyExecutive', 'proxyCoordinator', 'proxySumAssured', 'proxyCoverage', 'proxyConditioned', 'proxyMedicine', 'proxyCompanies',
    function(proxyGeneral, proxyLookup, proxyClinic, proxyAffiliate, proxyDiagnostic, proxyDoctor, 
      proxyMedicalCare, proxyCopay, proxyGuaranteeLetter, $window, proxyExternalAudit, $http, $q, proxyMenu, proxyTipoDocumento, proxyInternalRemark, 
      proxyExternalAuditor, proxyEps, proxyMedicalAuditor, proxyCompany, proxyCause, proxyLocation, proxyMobility, proxyDocument, proxyPolicy, proxySex, proxyPolicySoat,
      proxyPowerEpsProduct, proxyPowerEpsAffiliate, proxyProduct, proxyCoverageMinsa, proxySinisterExistence, proxyHighCosts, proxyExecutive, proxyCoordinator, proxySumAssured, proxyCoverage, proxyConditioned, proxyMedicine, proxyCompanies) {
...
/*    function getListBranchClinicByRuc(params, showSpin) {
    ...
    */
    ...
    function getTicketUser(params, showSpin) {
      //return proxyGrqcSecurity.Resource_Grqc_Security_GetTicketUser(params, showSpin);
    }
    ...
    function getListGRQCState(showSpin) {
      return proxyLookup.Resource_GRQC_Get_ListState(showSpin);
    }
...
//modal
    ...
    function downloadPlanillaAE(fileType, params, defaultFileName) {
      ...
      // };
      $http.post('http://api.oim.com/oim_grqc/api/externalaudit/template/PDF', params, { responseType: "arraybuffer" }).success(
    ...
    function getRole(params) {
      return proxyGrqcSecurity.Resource_Grqc_Security_GetRole(params, false);
    }
    ...
    function GetAllProductBy(userCode, systemCode) {
      console.log("Dentro a Obtener Productos");
    ...
    // Listado de registros de bandejaRequerimientos
    ...
    function GetListGRQCScanState(showSpin) {
    ...
    return {
      ...
      restarMes: restarMes,
      getListGRQCState: getListGRQCState,
     ...
     // downloadAdjuntoAE: downloadAdjuntoAE,
      ...
      /* Reporte SCAN */
      // Obtener lista de estados scan
      GetListGRQCScanState: GetListGRQCScanState,
  ...
  appGrqc.service('fileUpload', ['$http', '$q', 'mpSpin', function ($http, $q, mpSpin) {
    this.solicitarCartaURL = function(file, paramsCarta) {
      ...
      console.log("constants.system.api.endpoints.grqc");
      console.log(constants.system.api.endpoints.grqc);
      $http.post(constants.system.api.endpoints.grqc+ 'api/guaranteeletter/save', fd, {
  ...
  appGrqc.service('stepsService', function() {
    var step = [];
    var step1 = [];
    var step2 = [];
    var step3 = [];
