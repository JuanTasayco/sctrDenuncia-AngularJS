(function($root, deps, factory){
  define(deps, factory)
    ...
    var configChart = angular.module('oim.constants.atencionsiniestrosagricola', []);
    configChart.constant('config', {
      ...
      cartas: {
        ...
        numeroCgRechazadas: {
          ...
          type: 'text',
          valueIsNumber: true,
          title: 'Cartas de garantía rechazada',
          fields: {
            value: 'ncgrchzds'
          }
        },
        ...
        motivoRechazo: {
          ...
          table: {
            dmrchzo: 'Cobertura',
            pcgrchzds: 'Porcentaje de cartas de garantía aprobadas',
        ...
        return {
          ...
          'KpiTiempoAtencionSiniestro' : function(filters, product, data, umbral){
            ...
            if(!filters.ac_frclmo.cdgo){
              ...
              var guide1 = {umbral: umbral.credito };
              var guide2 = {};
              if(product.cdgo == 'O') guide2 = { umbral: 10 };
              else guide2 = {umbral: umbral.reembolso };
              ...
              configJson.guides.simple.umbral = umbral.credito;
            } else if(filters.ac_frclmo.cdgo == 'R') {
              configJson.title = title + ' por Reembolso';
              configJson.data = data.reembolso;
              if(product.cdgo == 'O') configJson.guides.simple.umbral = 10;
          ...
          'convertTasaImporteHospitalizacion': function(data){
            ...
            for(var i = 0; i < data.porcentaje.length; i++){
              ...
              var ihcbrtra = total != 0 ? (Math.round((parseFloat(porcentaje.ihcbrtra) / total) * 100 * 100) / 100) : 0;
              var ihotrs = total != 0 ? (Math.round((parseFloat(porcentaje.ihotrs) / total) * 100 * 100) / 100) : 0;
          ...
          'convertSoatGastosProveedor': function(data){
            ...
            for(var i = 0; i < data.length; i++){
              ...
              if(item[gastosPorProveedor.fields.value] !== '' && item[gastosPorProveedor.fields.value] !== '0'){
                var importe = parseFloat(item[gastosPorProveedor.fields.value]);
                total += importe;
              }
            }
          ...
          'convertSoatGastosCobertura': function(data){
            ...
            for(var i = 0; i < data.length; i++){
              ...
              if(item[gastosPorCobertura.fields.value] !== '' && item[gastosPorCobertura.fields.value] !== '0'){
                var importe = parseFloat(item[gastosPorCobertura.fields.value]);
                total += importe;
              }
            }
          ...
          'convertSoatGastosTipoAccidente': function(data){
            ...
            for(var i = 0; i < data.length; i++){
              ...
              if(item[gastosPorAccidente.fields.value] !== '' && item[gastosPorAccidente.fields.value] !== '0'){
                var importe = parseFloat(item[gastosPorAccidente.fields.value]);
                total += importe;
              }
            }
          ...
          'convertSoatGastosUbicacionGeograficaGeneral': function(data){
            ...
            for(var i = 0; i < data.length; i++){
              ...
              if(item[gastosPorUbicacionGeneral.fields.value] !== '' && item[gastosPorUbicacionGeneral.fields.value] !== '0'){
                var importe = parseFloat(item[gastosPorUbicacionGeneral.fields.value]);
                total += importe;
              }
            }
            ...
            for(var i = 0; i < data.length; i++){
              ...
              for(var key in numeroTotalCg.legends){
                if(item[key] !== '' && item[key] !== '0'){
                  var valueItem = parseFloat(item[key]);
                  total += valueItem;
                }
              }
              for(var key in numeroTotalCg.legends){
          ...
          'convertDiagnosticoAprobado': function(data){
            ...
            for(var i = 0; i < data.length; i++){
              ...
              if(item[diagnosticoAprobado.fields.base] !== '' && item[diagnosticoAprobado.fields.base] !== '0'){
                var importe = parseFloat(item[diagnosticoAprobado.fields.base]);
                total += importe;
              }
            }
          ...
          'convertCoberturaAprobada': function(data){
            ...
            for(var i = 0; i < data.length; i++){
              ...
              if(item[coberturaAprobada.fields.base] !== '' && item[coberturaAprobada.fields.base] !== '0'){
                var importe = parseFloat(item[coberturaAprobada.fields.base]);
                total += importe;
              }
            }
          ...
          'validarTasaHospitalizacion': function(filters){
            var validFormaReclamo = filters.ac_frclmo.cdgo == 'C';
            var validCobertura = filters.ac_ccbrtra == undefined || filters.ac_ccbrtra.cdgo == null || filters.ac_ccbrtra.cdgo == '3122';
            return validFormaReclamo && validCobertura;
          },
          'validarCostoPacienteMes': function(product){
            return product.cdgo == 'S';
          },
          ...
          'validateKeyInFilter': function(dashboard, product, filters, key){
            ...
            if(key == 'ac_cadtrmdco' && !isSoat) return false;
          ...
          'generatePdfFromChart': function(chart, data, table, filters, username, config, callback){
            ...
            $templateRequest(templateUrl).then(function(template) {
              ...
              html = html.replace('{{tipoProducto}}', config.product);
              html = html.replace('{{filtros}}', filters);
              html = html.replace('{{nombreGrafico}}', config.title);
              html = html.replace('{{imagenGrafico}}', chart);

              if(config.subtitle !== ''){
                divSubtitle = '<div class="atencionsiniestrosagricola-chart__second-title"><b>' + config.subtitle + '</b></div>';
              }
              ...
              for(var i = 0; i < headers.length; i++){
                ...
                headersTable += '<th class="atencionsiniestrosagricola-report__table-header">'+ header.label + '</th>';
              }
              ...
              for(var i = 0; i < data.length; i++){
                ...
                for(var j = 0; j < headers.length; j++){
                  ...
                  rowsTable += '<td class="atencionsiniestrosagricola-report__table-item">'+ value + '</td>';
                }
                rowTable += '</tr>';
                rowsTable += rowTable;
              }
              ...
              for(var page = 1; page <= totalPage; page++){
                ...
                for(var i = offset; i < numItems; i++){
                  ...
                  if(i < config.numHastTextCharts){
                    chartText += '<div class="atencionsiniestrosagricola-report__chart-text-container""><div class="atencionsiniestrosagricola-chart__canvas-container">';
                    chartText += '<div class="atencionsiniestrosagricola-report__chart-text-value">' + chart.image + '</div></div>';
                    chartText += '<div class="atencionsiniestrosagricola-chart__title-container">';
                    chartText += '<div class="atencionsiniestrosagricola-chart__text-title atencionsiniestrosagricola-chart__title-only">' + chart.title + '</div></div></div>';
                    if(i == config.numHastTextCharts - 1) {
                      chartText += '</div>';
                      chartImage += chartText;
                    }
                  } else {
                    chartImage += '<div class="col-md-6 col-sm-6"><div class="atencionsiniestrosagricola-report__chart-container">';
                    chartImage += '<div class="atencionsiniestrosagricola-chart__title-container"><div class="atencionsiniestrosagricola-report__chart-title"><div>' + chart.title + '</div>'
                    if(chart.subtitle !== ''){
                      chartImage += '<div class="atencionsiniestrosagricola-chart__second-title">' + chart.subtitle + '</div>'
                    }
                    chartImage += '</div></div>';
                    chartImage += '<div><img style="width:100%;" src="' + chart.image + '"></div>';
                    chartImage += '</div></div>';
                  }
