...
define(['angular', 'lodash'], function (ng, _) {
  ReGastosSepelioModalController.$inject = ['reFactory', '$log', '$window', 'reServices'];

  function ReGastosSepelioModalController(reFactory, $log, $window, reServices) {
    var vm = this;
    vm.$onInit = onInit;
    ...
    function onInit() {
      ...
      vm.frmCoverage = {}
      vm.frmCoverage.liquidationType = {
        codeField: 'N'
      }
    }

    function onChanges(changes) {
      if (changes.frmData && vm.frmData) {
    ...
    function setDestinatario() {
      ...
      vm.frmCoverage.destinatario = lastName.toUpperCase() + ' ' + motherLastName.toUpperCase() + ' ' + names.toUpperCase();
    }
  ...
  return ng
    .module('appReembolso')
    .controller('ReGastosSepelioModalController', ReGastosSepelioModalController)
    .component('reGastosSepelioModal', {
      templateUrl: '/reembolso/app/components/solicitud/steps/coverages/coverages-modal-form/gastos-sepelio-modal/gastos-sepelio-modal.html',
      controller: 'ReGastosSepelioModalController as $ctrl',
      bindings: {
        callSave: '<',
        closeModal: '<',
        documentLiquidation: '<',
